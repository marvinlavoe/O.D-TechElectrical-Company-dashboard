...
import { createLocalId, fromMinorUnits, nowIso, toMinorUnits } from '../lib/localData'
...
export async function listDocuments(type) {
  const result = await queryDatabase(`${documentSelect} ${type ? 'WHERE d.type = ?' : ''} ORDER BY d.created_at DESC`, type ? [type] : [])
  return (result.values || []).map((row) => mapDocument({ ...row, items: parseItems(row) }))
}
  ...
  return withTransaction(async (db) => {
    ...
    await db.run(
      `INSERT INTO billing_documents
       (id, document_number, customer_id, type, date, amount_minor, status, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, documentNumber, form.customer_id, form.type, form.date, toMinorUnits(form.amount), form.status, timestamp, timestamp],
    )
    ...
    for (const item of form.items) {
      await db.run(
        `INSERT INTO document_items (id, document_id, description, qty, price_minor)
         VALUES (?, ?, ?, ?, ?)`,
        [createLocalId(), id, item.description.trim(), Number(item.qty), toMinorUnits(item.price)],
      )
    }

    return { ...form, id, document_number: documentNumber, created_at: timestamp, updated_at: timestamp }
  })
}
  ...
  return withTransaction(async (db) => {
    await db.run(
      `UPDATE billing_documents
       SET customer_id = ?, date = ?, amount_minor = ?, status = ?, updated_at = ?
       WHERE id = ?`,
      [form.customer_id, form.date, toMinorUnits(form.amount), form.status, timestamp, id],
    )
