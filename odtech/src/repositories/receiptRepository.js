  ...
  return withTransaction(async (db) => {
    ...
    await db.run(
      `INSERT INTO receipts
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, receiptNumber, form.customer_id, form.job_id || null, form.date, toMinorUnits(form.amount), form.method, form.notes || null, timestamp, timestamp],
    )
    return { ...form, id, receipt_number: receiptNumber, created_at: timestamp, updated_at: timestamp }
  })
}
...
export async function updateReceipt(id, form) {
  ...
  await runDatabase(
    `UPDATE receipts
     ...
     WHERE id = ?`,
    [form.customer_id, form.job_id || null, form.date, toMinorUnits(form.amount), form.method, form.notes || null, timestamp, id],
  )
