...
export default function BillingPage() {
  ...
  const handleCreate = async (form) => {
    ...
    const count =
      (formType === "Invoice" ? invoices.length : quotes.length) + 1;
      ...
      .insert([
        {
          ...
          date: form.date,
          amount: form.amount,
          workmanship_cost: form.workmanship_cost,
      ...
      render: (_, row) => (
        <div className="flex gap-2">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => handleOpenDrawer(row.type, row)}
          >
          ...
          <Button
            variant="ghost"
            size="sm"
            onClick={() => handleGeneratePDF(row, row.type)}
          >
  ...
  const paymentColumns = [
    ...
    { key: "date", header: "Date", render: (val) => formatDate(val) },
    {
      ...
      header: "Amount Paid",
      render: (val) => (
        <span className="font-bold text-success">{formatCurrency(val)}</span>
      ),
    },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-surface-border bg-surface-card">
        ...
        {loading ? (
          ...
          <>
            {activeTab === "invoices" && (
              <DataTable data={invoices} columns={docColumns} />
            )}
            {activeTab === "quotes" && (
              <DataTable data={quotes} columns={docColumns} />
            )}
            {activeTab === "payments" && (
              <DataTable data={[]} columns={paymentColumns} />
            )}
          </>
        )}
      </div>
